...
>
> Đây là case "gãy khúc" nhất series: đường tấn công kinh điển (Bronze Bit) **đâm thẳng vào một bản vá của Microsoft và chết**, buộc phải quay xe sang đúng con đường mà tác giả GOAD thiết kế (RBCD chaining). Chính vì thế nó dạy nhiều thứ hơn một case "chạy lệnh là ra": phân biệt lỗi client vs lỗi server, cách research để xác minh giả thuyết, và — quan trọng nhất cho SOC — một **audit gap kép** khiến hành vi tấn công cốt lõi vô hình nếu không cấu hình đúng.
...
**Bài học đọc lỗi (quan trọng cho SOC):**
...
Đúng tinh thần Case 3, không tin ngay giả thuyết mà đưa toàn bộ context cho 2 công cụ research độc lập (Perplexity + Gemini). Cả hai hội tụ:
...
- Nhưng Gemini phản biện một điểm tôi kết luận sai: *"force-forwardable chết" ≠ "hướng này chết"*. Con đường **đúng ý đồ GOAD** là **RBCD chaining**, trích thẳng blog tác giả (mayfly277, GOADv2 part10):
...
Tức GOAD **cố ý dùng Server 2019 đã vá** để chặn Bronze Bit, ép người học đi đường RBCD chaining.

## Bước 5 — Quay xe: RBCD chaining (đường chính danh)

**Ý tưởng vượt patch:** không lật cờ nữa (sẽ bị bắt), mà **lấy một vé forwardable HỢP LỆ do chính KDC ký**, nhờ một bất đối xứng nổi tiếng (Elad Shamir — *Wagging the Dog*): **RBCD ngầm cấp "protocol transition"** — S4U qua đường RBCD trả về vé **forwardable thật**, kể cả khi evidence ban đầu non-forwardable.
